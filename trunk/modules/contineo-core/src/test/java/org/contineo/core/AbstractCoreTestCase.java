 ...
 * <p>
 * All Contineo's tests must extend this test case in order to find a ready and
 * accessible database.
 * 
...
public abstract class AbstractCoreTestCase extends TestCase {
	...
	static {
		System.setProperty("CONTINEO_HOME", "target");
	}
	...
	protected void createTestDirs() throws IOException {
		...
		dbSchemaFile = new File(tempDir, "contineo-core.sql");
		dataFile = new File(tempDir, "data.sql");

		// Copy sql files
		copyResource("/org/contineo/core/contineo-core.sql", dbSchemaFile.getCanonicalPath());
