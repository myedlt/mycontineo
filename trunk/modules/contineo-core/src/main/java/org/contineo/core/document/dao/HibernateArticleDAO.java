...
public class HibernateArticleDAO extends HibernateDaoSupport implements ArticleDAO {
	...
	public Collection<Article> findByDocId(int docId) {
		...
		try {
			coll = (Collection<Article>) getHibernateTemplate()
					.find(
							"from org.contineo.core.document.Article _article where _article.docId = ? order by _article.articleDate",
	...
	public Collection<Article> findByUserName(String username) {
		...
		try {
			coll = (Collection<Article>) getHibernateTemplate()
					.find(
							"from org.contineo.core.document.Article _article where _article.username = ? order by _article.articleDate",
							new Object[] { username });
		} catch (Exception e) {
			if (log.isErrorEnabled())
				logger.error(e.getMessage(), e);
